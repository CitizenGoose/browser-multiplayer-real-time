/*
This file defines entity hirearchies like Player and Bullet.
*/
require('./app');
...
Bullet = function(param){
	...
	var super_update = self.update;
	self.update = function(){
		if(self.timer++ > 100) // range
			self.toRemove = true;
		super_update();
		// Bullet damage
		for(var i in Player.list){ // for every player
			var p = Player.list[i]; // reference to each player one by one

			// This checks 3 things. 1. map of bullet = map of player. 2. distance is less than 32px. 3. player is not the parent
...
Wall.getAllInitPack = function(){
	var walls = [];
	for(var i in Wall.list)
		walls.push(Wall.list[i].getInitPack());
	return walls;
}
///////////////////////////////////////////////////////////////////////////////
